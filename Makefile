...
MIGRATION_PATH = ./postgres/migration
...
postgres-test: network-test
	@docker run --rm --name $(DB_NAME)-test-db --network=$(DB_NAME)-test-net -p 5432:5432 -e POSTGRES_USER=$(DB_USER) -e POSTGRES_PASSWORD=$(DB_PASS) -e POSTGRES_DB=$(DB_NAME) -d postgres:12-alpine
