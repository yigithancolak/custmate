...
func (s *Store) UpdateCustomer(tx *sql.Tx, customerID string, input *model.UpdateCustomerInput) (*model.Customer, error) {
	...
	returnQuery := " RETURNING name, phone_number, last_payment, next_payment, active"
	var updates []string
	var args []interface{}
	...
	err := tx.QueryRow(query, args...).Scan(&customer.Name, &customer.PhoneNumber, &customer.LastPayment, &customer.NextPayment, &customer.Active)
	if err != nil {
		return nil, err
	}
